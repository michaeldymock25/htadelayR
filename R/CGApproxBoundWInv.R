...
CGApproxBoundWInv <- function(mval){
  ...
  #
  # OUTPUTS:
  # sval : set of s values which correspond to inverting boundary: sval = b-wiggle-inverse(mval)
  ...
  sval[mval <= 0] <- Inf                                        # nonpositive values have infinite inverse
  sval[mval <= breakpnt1] <- sqrt(2)*mval[mval <= breakpnt1]  # first patch can do analytically

  if(any(mval > breakpnt1 & mval <= breakpnt2)){  # if there are points in the middle patch...
    loval <- 0.99 * kink1                         # pick values just outside of band for middle patch
    hival <- 1.01 * kink2                         # just in case there is a bit of roundoff at the kinks
    numsteps <- 600
    ...
    # stopping times in this middle band (relative to stopping times in the CG paper).
    bndsvec <- exp(-0.0275*log(myvec)^2 + 0.8797*log(myvec) - 0.5024)  # needs to match formula in CFApproxBoundW
    sval[mval > breakpnt1 & mval <= breakpnt2] <- approx(bndsvec, myvec, xout = mval[mval > breakpnt1 & mval <= breakpnt2])$y
  }

  if(any(mval > breakpnt2)){
    # if there are points in the third patch
    ...
    loval <- 0.99*kink2  # pick values just below the third patch
    hival <- kink3       # just in case there is a bit of roundoff at the kinks
    ...
    sval[mval > breakpnt2] <- approx(bndsvec, myvec, xout = mval[mval > breakpnt2])$y
  }
