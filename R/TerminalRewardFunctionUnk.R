...
TerminalRewardFunctionUnk <- function(valuevec, discountfactor, predvarsample = NULL, nochangeallowed = FALSE, dof = NULL){
  ...
  if(is.null(dof)) stop("TerminalRewardFunction: not enough arguments")
  ...
  if(predvarsample > 0){
    ...
    rval <- discountfactor^(1 - nochangeallowed)*(sqrt(predvarsample)*PsiNormUV(zvec, dof) + valuevec*(valuevec > 0))
