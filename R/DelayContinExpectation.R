...
DelayContinExpectation <- function(first, last, pu, pd, psame, myvec){
  ...
  rval <- tryCatch({pu*myvec[(first+1):(last+1)] + psame*myvec[first:last] + pd*myvec[(first-1):(last-1)]},
                   error = function(e){
                             warning(paste("Error in index bounds: first =", first, ", last =", last, ", length(myvec) =", length(myvec)))
                             return(myvec)})
